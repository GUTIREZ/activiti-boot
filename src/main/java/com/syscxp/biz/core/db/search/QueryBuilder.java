...
public class QueryBuilder {
    ...
    private JSONObject term(String name, String val) {
        try {
            return (JSONObject) nj().put("term", nj().put(name, val));
    ...
    private JSONObject notTerm(String name, String val) {
        try {
            return (JSONObject) nj().put("not", nj().put("term", nj().put(name, val)));
        } catch (JSONException e) {
            throw new ServiceException(e.getMessage());
        }
    }
    ...
    private JSONObject gt(String name, String val) {
        try {
            return (JSONObject) nj().put("range", nj().put(name, nj().put("gt", val)));
        } catch (JSONException e) {
            throw new ServiceException(e.getMessage());
        }
    }
    ...
    private JSONObject gte(String name, String val) {
        try {
            return (JSONObject) nj().put("range", nj().put(name, nj().put("gte", val)));
        } catch (JSONException e) {
            throw new ServiceException(e.getMessage());
        }
    }
    ...
    private JSONObject lt(String name, String val) {
        try {
            return (JSONObject) nj().put("range", nj().put(name, nj().put("lt", val)));
        } catch (JSONException e) {
            throw new ServiceException(e.getMessage());
        }
    }
    ...
    private JSONObject lte(String name, String val) {
        try {
            return (JSONObject) nj().put("range", nj().put(name, nj().put("lte", val)));
        } catch (JSONException e) {
            throw new ServiceException(e.getMessage());
        }
    }
    ...
    private JSONObject in(String name, List<String> vals) {
        try {
            return (JSONObject) nj().put("terms", nj().put(name, vals));
        } catch (JSONException e) {
            throw new ServiceException(e.getMessage());
        }
    }
    ...
    public JSONObject notIn(String name, List<String> vals) {
        try {
            return (JSONObject) nj().put("not", nj().put("terms", nj().put(name, vals)));
        } catch (JSONException e) {
            throw new ServiceException(e.getMessage());
        }
    }
    ...
    public String build() {
        try {
            ...
            if (!andList.isEmpty()) {
                ...
                globalOr.add((JSONObject) nj().put("and", andList));
                isFilter = true;
            }
            if (!orList.isEmpty()) {
                //filterObj.put("or", orList);
                globalOr.add((JSONObject) nj().put("or", orList));
                isFilter = true;
            }
            if (isFilter) {
                JSONObject filterObj = (JSONObject) nj().put("or", globalOr);
