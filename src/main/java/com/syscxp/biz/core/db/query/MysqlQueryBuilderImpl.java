...
import com.syscxp.biz.core.db.search.header.Parent;
import com.syscxp.biz.core.db.search.header.TypeField;
import com.syscxp.biz.core.utils.TypeUtils;
import com.syscxp.biz.core.utils.zstack.BeanUtils;
import com.syscxp.biz.core.utils.zstack.CollectionDSL;
import com.syscxp.biz.core.utils.zstack.CollectionUtils;
...
import com.syscxp.biz.core.utils.zstack.gson.JSONObjectUtil;
...
import static com.syscxp.biz.core.utils.zstack.StringDSL.s;
 ...
 */
@Component
public class MysqlQueryBuilderImpl implements QueryBuilder {
    private static final Logger logger = LoggerFactory.getLogger(MysqlQueryBuilderImpl.class);
    ...
    // 格式化查询数据
    public QueryMessage intercept(QueryMessage msg) {
    ...
    private class ExpandedQueryAliasInfo {
        ...
        void check() {
            ...
            if (isFromAnnotation) {
                ...
                throw new RuntimeException(String.format("inventory[%s] has an expanded query alias[%s]," +
                ...
                throw new RuntimeException(String.format("inventory[%s] has an expanded query alias[%s](added by AddExpandedQueryExtensionPoint]," +
    ...
    private class EntityInfo {
        ...
        EntityInfo(Class invClass) throws NoSuchMethodException {
            ...
            if (!entityClass.isAnnotationPresent(Entity.class)) {
                throw new RuntimeException(String.format("class[%s] is not annotated by @Entity, but it's stated as entity class by @Inventory of %s",
            ...
            if (jpaMetaClass == null) {
                throw new RuntimeException(String.format("cannot find JPA meta model class for entity class[%s], the meta model class is expected as %s",
        ...
        Class selectInventoryClass(final QueryMessage msg) {
            if (inventoryTypeField == null) {
    ...
    private void populateEntityInfo() throws NoSuchMethodException {
        List<Class> metaClasses = BeanUtils.scanClass("com.syscxp.biz", StaticMetamodel.class);
        ...
        List<Class> invClasses = BeanUtils.scanClass("com.syscxp.biz", Inventory.class);
    ...
    private class MetaCondition {
        ...
        private Class getEntityFieldType() {
            if (Collection.class.isAssignableFrom(entityField.getType())) {
                ...
                throw new RuntimeException(String.format("query cannot support Map type. %s.%s",
        ...
        private Object doNormalizeValue(String value) {
            try {
                ...
                throw new RuntimeException(String.format("failed to parse value[%s]", value), e);
            }
        }
        ...
        String toJpql() {
            ...
            if (!skipInventoryCheck) {
                if (inventoryField == null || inventoryField.isAnnotationPresent(APINoSee.class)) {
                    throw new RuntimeException(String.format("condition name[%s] is invalid, no such field on inventory class[%s]",
                            attr, inventoryClass.getName()));
                }

                if (inventoryField.isAnnotationPresent(Unqueryable.class)) {
                    throw new RuntimeException(String.format("condition name[%s] is invalid, field[%s] of inventory[%s] is annotated as @Unqueryable field",
            ...
            if (at == null) {
                ...
                if (metaField == null) {
                    throw new RuntimeException(String.format("entity meta class[%s] has no field[%s]",
    ...
    private class QueryObject {
        ...
        class TagSqlBuilder {
            ...
            private String chooseOp(QueryCondition cond) {
                ...
                throw new RuntimeException(String.format("invalid comparison operator[%s]; %s", cond.getOp(), JSONObjectUtil.toJsonString(cond)));
            }
    ...
    private class QueryContext {
        ...
        private void validateFields() {
            ...
            for (String f : msg.getFields()) {
                if (!info.premitiveFieldNames.contains(f)) {
                    throw new RuntimeException(String.format("field[%s] is not a primitive of the inventory %s; you cannot specify it in the parameter 'fields';" +
        ...
        private List convertFieldsTOPartialInventories(List fieldTuple) {
            ...
            for (Object t : fieldTuple) {
                Map<String, Object> map = new HashMap<>(fieldTuple.size());
                Tuple tuple = (Tuple) t;
        ...
        for (Class invClass : invClasses) {
            ...
            for (ExpandedQueryAlias alias : aliases.value()) {
                ...
                if (info.queryMessageClass == null) {
                    throw new RuntimeException(String.format("inventory[%s] declares expanded query alias, but not query message declare this inventory class in AutoQuery annotation",
                            invClass.getName()));
                }
    ...
    public boolean init() {
        try {
            List<Class> queryMessageClasses = BeanUtils.scanClassByType("com.syscxp.biz", QueryMessage.class);
    ...
    private Class selectInventoryClass(QueryMessage msg, Class inventoryClass) {
        EntityInfo info = entityInfos.get(inventoryClass);
    ...
    public Map<String, List<String>> populateQueryableFields() {
        ...
        class QueryableBuilder {
            ...
            private void buildInherentQueryableFields() {
                for (Field field : info.allFieldsMap.values()) {
                    ...
                    if (TypeUtils.issyscxpBeanPrimitive(field.getType())) {
                    ...
                    if (Collection.class.isAssignableFrom(field.getType()) && field.isAnnotationPresent(Queryable.class)) {
                        ...
                        if (!gtype.isInferred()) {
                            throw new RuntimeException(String.format("unable infer generic type of %s.%s", field.getDeclaringClass(), field.getName()));
                        }
                        if (gtype.getNestedGenericValue() != null) {
                            throw new RuntimeException(String.format("%s.%s is nested Collection, not support", field.getDeclaringClass(), field.getName()));
                        }
                        if (TypeUtils.issyscxpBeanPrimitive(gtype.getValueType())) {
                            queryableFields.add(field.getName());
                            continue;
                        }
                        Class nestedInventory = gtype.getValueType();
                        if (!nestedInventory.isAnnotationPresent(Inventory.class)) {
                            throw new RuntimeException(String.format("field[%s] on inventory class[%s] is collection type with @Queryable, but its generic type[%s] is not an inventory class",
        ...
        for (Entry<Class, Class> e : inventoryQueryMessageMap.entrySet()) {
