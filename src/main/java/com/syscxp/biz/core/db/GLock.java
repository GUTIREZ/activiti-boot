...
public class GLock {
    private static final Logger logger = LoggerFactory.getLogger(GLock.class);
    ...
    private void checkInThread() {
        ...
        if (locks.contains(name)) {
            throw new ServiceException(String.format("Thread[%s] has acquired lock[%s], you can NOT acquire the lock again before unlock, GLock is non reentrant",
    ...
    public void lock() {
        ...
        try {
            logger.debug(String.format("[GLock]: thread[%s] is acquiring lock[%s]", Thread.currentThread().getName(), name));

            if (separateThreadEnabled) {
                mlock.lock();
                logger.debug(String.format("[GLock Memory Lock]: thread[%s] got memory lock[%s]", Thread.currentThread().getName(), name));
            }
            ...
            try {
                ...
                if (rs == null) {
                    ...
                    throw new ServiceException(String.format("lock[%s] failed, timeout after %s seconds", name, timeout));
                }


                logger.debug(String.format("[GLock DB Lock]: thread: %s got DB lock[%s], during timeout[%s secs]", Thread.currentThread().getName(), name, timeout));

            } catch (SQLException e) {
                throw new ServiceException(String.format("[GLock Error]: cannon get DB connection for lock[%s]", name), e);
            } finally {
                if (pstmt != null) {
                    try {
                        pstmt.close();
    ...
    public void unlock() {
        if (!success) {
            logger.debug(String.format("[GLock]: skip unlock for thread[%s] on lock[%s], because previous lock() is not success",
                    Thread.currentThread().getName(), name));
        ...
        try {
            ...
            logger.debug(String.format("[GLock]: thread[%s] is releasing lock[%s]", Thread.currentThread().getName(), name));
            ...
            try {
                ...
                ResultSet rs = pstmt.executeQuery();
                if (rs == null) {
                    throw new ServiceException("Mysql cannot find lock: " + name);
                } else if (rs.first() && rs.getInt(1) == 0) {
                    String err = "Unable to release DB lock: " + name + ", lock: " + name + " is not held by this connection, internal error";
                    throw new ServiceException(err);
                }

                logger.debug(String.format("[GLock Release DB Lock] thread[%s] released DB lock[%s]", Thread.currentThread().getName(), name));

            } catch (SQLException e) {
                throw new ServiceException("Unable to release lock: " + name, e);
            } finally {
                if (pstmt != null) {
                    try {
                        pstmt.close();
            ...
            logger.debug(String.format("[GLock Release Memory Lock]: thread[%s] released memory lock[%s]", Thread.currentThread().getName(), name));
