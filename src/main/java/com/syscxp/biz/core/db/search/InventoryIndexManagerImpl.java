...
import com.syscxp.biz.core.db.TransactionalCallback;
import com.syscxp.biz.core.db.search.header.SqlTrigger;
import com.syscxp.biz.core.db.search.header.SqlTriggers;
import com.syscxp.biz.core.exception.ServiceException;
import com.syscxp.biz.core.db.search.header.*;
import com.syscxp.biz.core.utils.zstack.CollectionUtils;
import com.syscxp.biz.core.utils.zstack.data.Pair;
...
@Component
public class InventoryIndexManagerImpl implements InventoryIndexManager, TransactionalCallback {
    private static final Logger logger = LoggerFactory.getLogger(InventoryIndexManagerImpl.class);
    ...
    public Set<String> getBasePkgNames() {
        if (basePkgNames == null) {
            ...
            basePkgNames.add("com.syscxp.biz");
        }
        return basePkgNames;
    }
        ...
        root.put(
                ...
                new JSONObject().put(
                        ...
                        new JSONObject().put("analysis",
                                new JSONObject().put("analyzer", new JSONObject().put("default", new JSONObject().put("type", "keyword"))))));
        return root;
    }
    ...
    private ESBulkBuilder addDocToIndexToESBuilder(ESBulkBuilder bbuilder, Map<String, Set<String>> vmap) {
        try {
            for (Entry<String, Set<String>> e : vmap.entrySet()) {
    ...
    private ESBulkBuilder addDocToDeleteToESBuilder(ESBulkBuilder bbuilder, Map<String, Set<String>> vmap) {
        for (Entry<String, Set<String>> et : vmap.entrySet()) {
    ...
    private IndexerInfo getIndexerInfoByInventoryName(String inventoryName) {
        ...
        throw new ServiceException(String.format("cannot find IndexerInfo for inventory[%s]", inventoryName));
    }
    ...
    private Field getEntityIdFieldFromClass(Class<?> clazz) {
        ...
        throw new ServiceException(String.format("class[%s] doesn't have a field annotated by %s, is it a JPA @Entity class???", clazz.getName(), Id.class.getName()));
    }
    ...
    private void generateParentSqlTriggerInheritance(Class<?> clazz) {
        ...
        do {
            if (!parent.isAnnotationPresent(Inheritance.class)) {
                throw new ServiceException(String.format("@Entity class %s inherits %s, but %s doesn't have @Inheritance annotation", clazz.getName(), parent.getName(), parent.getName()));
            }
    ...
    private void doGenerateSqlTriggerText(SqlTrigger trigger, Class<?> clazz, StringBuilder sb) {
        ...
        if (trigger.foreignVOClass() != Object.class) {
            ...
            if (foreignVOUuid.equals("")) {
                throw new ServiceException(String.format("@SqlTrigger of %s has foreignVOClass set to %s, but foreignVOJoinColumn is empty", clazz.getName(), trigger.foreignVOClass().getName()));
            }
        ...
        if (trigger.foreignVOToDeleteClass() != Object.class) {
            ...
            if (foreignVOToDeleteUuid.equals("")) {
                throw new ServiceException(String.format("@SqlTrigger of %s has foreignVOToDeleteClass set to %s, but foreignVOToDeleteJoinColumn is empty", clazz.getName(), trigger.foreignVOToDeleteClass().getName()));
            }
        ...
        sb.append(String.format("\nDROP TRIGGER IF EXISTS %s;", insertTriggerName));
        sb.append(String.format("\nCREATE TRIGGER `syscxp`.%s AFTER INSERT ON `syscxp`.`%s`", insertTriggerName, voName));
        sb.append(String.format("\nFOR EACH ROW BEGIN"));
        ...
        sb.append(String.format("\nDROP TRIGGER IF EXISTS `syscxp`.%s;", updateTriggerName));
        sb.append(String.format("\nCREATE TRIGGER `syscxp`.%s AFTER UPDATE ON `syscxp`.`%s`", updateTriggerName, voName));
        sb.append(String.format("\nFOR EACH ROW BEGIN"));
        if (foreignVONames != null) {
            for (String name : foreignVONames) {
        ...
        if (children != null) {
            for (Class<?> child : children) {
                ...
                if (!childVOId.equals(idName)) {
                    throw new ServiceException(String.format("%s inherits %s, but %s has @Id as %s, %s has @Id as %s", child.getName(), clazz.getName(), child.getName(), childVOId, clazz.getName(), idName));
                }
        ...
        sb.append(String.format("\nDROP TRIGGER IF EXISTS `syscxp`.%s;", deleteTriggerName));
        sb.append(String.format("\nCREATE TRIGGER `syscxp`.%s AFTER DELETE ON `syscxp`.`%s`", deleteTriggerName, voName));
        sb.append(String.format("\nFOR EACH ROW BEGIN"));
