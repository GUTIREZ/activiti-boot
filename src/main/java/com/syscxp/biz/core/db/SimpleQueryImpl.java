...
public class SimpleQueryImpl<T> implements SimpleQuery<T> {
    private static final Logger logger = LoggerFactory.getLogger(SimpleQueryImpl.class);
    ...
    SimpleQueryImpl(Class<T> vo) {
        _dbf = SpringContext.getBean(DatabaseFacade.class);
        _entityClass = vo;
    ...
    @Override
    public SimpleQuery<T> orderBy(SingularAttribute attr, SimpleQuery.Od order) {
